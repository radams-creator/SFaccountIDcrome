...
async function getTabUrl(tabId) {
  ...
  try {
    ...
    console.debug("Unable to get tab URL", tabId, error);
    return null;
  }
}
    ...
    const [result] = await chrome.scripting.executeScript({
      ...
      func: (element, regexSource) => {
        ...
        const checkNode = (node, depth = 0) => {
          ...
          return checkNode(getParent(node), depth + 1);
        };
      ...
      const results = await chrome.scripting.executeScript({
        ...
        func: (regexSource) => {
          ...
          const searchNodeForMatch = (node, seen = new Set(), depth = 0) => {
            if (!node || seen.has(node) || depth > 4000) {
              return null;
            }
            seen.add(node);
            ...
            if (node.shadowRoot) {
              childElements.push(...Array.from(node.shadowRoot.children ?? []));
            }

            if (node.children && node.children.length) {
              childElements.push(...Array.from(node.children));
            }

            if (node.assignedElements && typeof node.assignedElements === "function") {
              childElements.push(...node.assignedElements());
            }
          ...
          const hrefMatch =
            tryMatch(window.location?.href) ||
            tryMatch(safeDecode(window.location?.hash ?? "")) ||
            tryMatch(safeDecode(window.location?.pathname ?? ""));
          if (hrefMatch) {
            return hrefMatch;
          }
          ...
          const inspectStorage = (storage) => {
            ...
            try {
              for (let index = 0; index < storage.length; index += 1) {
                ...
                const keyMatch = tryMatch(key);
                if (keyMatch) {
                  return keyMatch;
                }
                const valueMatch = tryMatch(value);
                if (valueMatch) {
                  return valueMatch;
                }
              }
          ...
          for (const storage of storageSources) {
            const storageMatch = inspectStorage(storage);
            if (storageMatch) {
              return storageMatch;
            }
          }

          const cookieMatch = tryMatch(typeof document !== "undefined" ? document.cookie : null);
          if (cookieMatch) {
            return cookieMatch;
          }

          let historyStateString = null;
          try {
            const historyState = window.history?.state ?? null;
            if (typeof historyState === "string") {
              historyStateString = historyState;
            } else if (historyState) {
              historyStateString = JSON.stringify(historyState);
            }
          } catch (error) {
            historyStateString = null;
          }

          const historyMatch = tryMatch(historyStateString);
          if (historyMatch) {
            return historyMatch;
          }

          const windowNameMatch = tryMatch(window?.name ?? null);
          if (windowNameMatch) {
            return windowNameMatch;
          }

          const titleMatch = tryMatch(document?.title ?? null);
          if (titleMatch) {
            return titleMatch;
          }

          const htmlMatch = tryMatch(document.documentElement?.innerHTML ?? null);
          if (htmlMatch) {
            return htmlMatch;
          }
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  const tabId = tab.id;
  const knownTabUrl = typeof tab.url === "string" ? tab.url : null;
  const resolvedTabUrl = await getTabUrl(tabId);
  const sources = [info.linkUrl, info.srcUrl, info.frameUrl, info.pageUrl, knownTabUrl, resolvedTabUrl];
